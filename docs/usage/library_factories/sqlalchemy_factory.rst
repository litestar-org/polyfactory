...
By default, ``__set_foreign_keys__``, ``__set_relationships__`` and ``__set_association_proxy__`` attributes are set to ``False``.
...
This attribute is used to include all fields mapped to the SQLAlchemy `ForeignKey <https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKey>`_ in the resulting mock dictionary created by ``build`` / ``build_async`` methods.
...
This attribute is used to include all fields with the SQLAlchemy `relationship() <https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship>`_ in the resulting mock dictionary created by ``build`` / ``build_async`` methods.
...
.. note::
    In general, :ref:`ForeignKey fields <foreign_keys>` are not automatically generated by ``build`` / ``build_async``. This behavior can be changed either by setting ``__set_foreign_keys__ = True``, or by using ``create_sync``/ ``create_async`` methods, so SQLAlchemy ORM creates them.

Association Proxy
+++++++++++++++++

All SQLAlchemy fields mapped to ORM `Association Proxy <https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#module-sqlalchemy.ext.associationproxy>`_ class will be included in factory's fields by ``__set_association_proxy__`` attribute.
